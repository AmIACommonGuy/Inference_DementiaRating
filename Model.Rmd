...
```{r}
summary(vglm(formula = CDR_ord ~ EDUC_cat + SEX + Marriage + DEP + FEVALAGE + RES_PRIVATE + 
    EDUC_cat:FEVALAGE + EDUC_cat:RES_PRIVATE + RACE, family = cumulative("logitlink", 
    parallel = T), data = df))
```

```{r}
library(MASS)
MF <- polr(CDR_ord ~ EDUC_cat + SEX + Marriage + DEP + RACE + FEVALAGE + RES_PRIVATE + 
    EDUC_cat:FEVALAGE + EDUC_cat:RES_PRIVATE, data = df, method = 'logistic')
MF
```
...
```{r}
mf <- vglm(formula = CDR_ord ~ EDUC_cat + SEX + Marriage + DEP + FEVALAGE + 
...
library(gofcat)
logitgof(df$CDR, fitted(MF), ord= T)
#hosmerlem(MF, tables = T)
```
